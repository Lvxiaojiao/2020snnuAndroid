...
public class PicLoadActivity extends AppCompatActivity {
    ...
    void uploadBtn(){
        ...
        if(!CheckUtils.isEmpty(userId)){
            ...
            Intent intent = new Intent();
            intent.setClass(PicLoadActivity.this, LoginActivity_.class);
    ...
    @Background
    void getIcon(String userId){
        Json j = usrService.getMyAmtA(userId);
        updateView(j);
    }
    @UiThread
    void updateView(Json j){
        if(!CheckUtils.isEmpty(j) && j.isSuccess()){
            Map<String,Object> map =(Map<String,Object>) j.getObj();
            Object usrO= (Object)map.get("usr");
            if(!CheckUtils.isEmpty(usrO)){
                Map<String,Object> usrMap =(Map<String,Object>)usrO;
                //content.setText(String.valueOf(usrMap.get("cid")));
                //Toast.makeText(this, String.valueOf(usrMap.get("cid")), Toast.LENGTH_SHORT).show();
                String iconDb =  CheckUtils.isEmpty(usrMap.get("icon"))?"":String.valueOf(usrMap.get("icon"));
                String url = URLProtocol.ROOT+":"+ URLProtocol.PORT+"/"+ URLProtocol.PROJECT+"/"+ iconDb;
                loadImg(image,url);
            }
        } else {
            ...
            case IMAGE_REQUEST_CODE://这里的requestCode是我自己设置的，就是确定返回到那个Activity的标志
                if (resultCode == RESULT_OK) {//resultcode是setResult里面设置的code值
                    try {
                        ...
                        String url=URLProtocol.ROOT+":"+ URLProtocol.PORT+"/"+ URLProtocol.PROJECT+"/"+ URLProtocol.ACTION_ICONUPLOAD+"?cid="+userId;
                        upImg(url,file);
        ...
        call.enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(PicLoadActivity.this, "失败", Toast.LENGTH_SHORT).show();
                    }
                });
            }
            @Override
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if(!CheckUtils.isEmpty(jsonAll)){
                            ...
                            Toast.makeText(PicLoadActivity.this, "上传失败", Toast.LENGTH_SHORT).show();
                        }
