...
public class ImgUtil
{
	...
	*	加载图片
	 */
	public static void loadImg(Context context,final ImageView imageView, String url)
	{
	...
	*	加载圆角图片
	 */
	public static void loadShaderImg(Context context,final ImageView imageView, String url,final int outWidth,final int outHeight,final int radius)
	{
		OkHttpUtils.get(url)
				.tag(context)
				.execute(new BitmapCallback()
				{
					@Override
					public void onSuccess(Bitmap bitmap, okhttp3.Call call, Response response) {
						imageView.setImageBitmap(roundBitmapByShader(bitmap,outWidth,outHeight,radius));
					}
				});
	}
	/**
